...
def random_patient_bundle(amount):
	random_patients = RandomPatients()
	bundle = Bundle()
...
class RandomObservation:
	...
	def generate_observation(self):
		...
		#if(randint(0,1)==0):
		related, subject = random_dict_list_items(sequence_patient_map)
		# else:
		# 	related, subject = self.generate_empty_related()
		#observation_patient_map["Observation/"+str(id)] = subject
		status = self.random_status.next()
		code = self.random_loinc.next()
		observation = Observation (id, status, code, subject, related)
		return observation
...
class RandomSequence:
	...
	#	id = idGenerator.next_sequence()
		...
		sequence = Sequence (id, sequence_type, coordinateSystem, patient, reference_sequence, variant) #default DNA 
		return sequence
